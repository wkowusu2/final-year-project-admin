...
function Sidebar({ active, setActive }) { return <aside className="sidebar"><a className="brand" href="#top"><span className="brand-mark">RP</span><span><strong>RoadPulse</strong><small>COMMAND</small></span></a><nav className="nav-links">{[['overview', '◉', 'Network overview'], ['traffic', '⌁', 'Traffic intelligence'], ['reports', '△', 'Incident reports']].map(([id, icon, label]) => <button key={id} className={active === id ? 'active' : ''} onClick={() => setActive(id)}><span>{icon}</span>{label}</button>)}</nav><div className="sidebar-note"><span className="pulse"/><div><strong>Live operations</strong><small>Updates from matched driver GPS points</small></div></div></aside>; }
